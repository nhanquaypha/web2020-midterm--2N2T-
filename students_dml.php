...
function students_insert(){
	...
	sql('insert into `students` set       `regno`=' . (($data['regno'] !== '' && $data['regno'] !== NULL) ? "'{$data['regno']}'" : 'NULL') . ', `name`=' . (($data['name'] !== '' && $data['name'] !== NULL) ? "'{$data['name']}'" : 'NULL') . ', `course`=' . (($data['course'] !== '' && $data['course'] !== NULL) ? "'{$data['course']}'" : 'NULL') . ', `year`=' . (($data['year'] !== '' && $data['year'] !== NULL) ? "'{$data['year']}'" : 'NULL') . ', `academicyear`=' . (($data['academicyear'] !== '' && $data['academicyear'] !== NULL) ? "'{$data['academicyear']}'" : 'NULL'), $o);
	if($o['error']!=''){
		echo $o['error'];
...
function students_delete($selected_id, $AllowDeleteOfParents=false, $skipChecks=false){
	...
	$res = sql("select `regno` from `students` where `regno`='$selected_id'", $eo);
	$regno = db_fetch_row($res);
	$rires = sql("select count(1) from `Marks` where `student`='".addslashes($regno[0])."'", $eo);
	$rirow = db_fetch_row($rires);
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks){
		$RetMsg = $Translation["couldn't delete"];
		$RetMsg = str_replace("<RelatedRecords>", $rirow[0], $RetMsg);
		$RetMsg = str_replace("<TableName>", "Marks", $RetMsg);
		return $RetMsg;
	}elseif($rirow[0] && $AllowDeleteOfParents && !$skipChecks){
		$RetMsg = $Translation["confirm delete"];
		$RetMsg = str_replace("<RelatedRecords>", $rirow[0], $RetMsg);
...
function students_update($selected_id){
	...
	$o=array('silentErrors' => true);
	sql('update `students` set       `regno`=' . (($data['regno'] !== '' && $data['regno'] !== NULL) ? "'{$data['regno']}'" : 'NULL') . ', `name`=' . (($data['name'] !== '' && $data['name'] !== NULL) ? "'{$data['name']}'" : 'NULL') . ', `course`=' . (($data['course'] !== '' && $data['course'] !== NULL) ? "'{$data['course']}'" : 'NULL') . ', `year`=' . (($data['year'] !== '' && $data['year'] !== NULL) ? "'{$data['year']}'" : 'NULL') . ', `academicyear`=' . (($data['academicyear'] !== '' && $data['academicyear'] !== NULL) ? "'{$data['academicyear']}'" : 'NULL') . " where `regno`='".makeSafe($selected_id)."'", $o);
	if($o['error']!=''){
		echo $o['error'];
...
function students_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $ShowCancel = 0, $TemplateDV = '', $TemplateDVP = ''){
	...
	if(is_file(dirname(__FILE__).'/hooks/students.year.csv')){
		$year_data = addslashes(implode('', @file(dirname(__FILE__).'/hooks/students.year.csv')));
		$combo_year->ListItem = explode('||', entitiesToUTF8(convertLegacyOptions($year_data)));
		$combo_year->ListData = $combo_year->ListItem;
	}else{
		$combo_year->ListItem = explode('||', entitiesToUTF8(convertLegacyOptions("1ST;;2ND;;3RD;;4TH;;5TH")));
		$combo_year->ListData = $combo_year->ListItem;
	}
	...
	if($selected_id){
		...
		$combo_year->SelectedText = ( $_REQUEST['FilterField'][1]=='4' && $_REQUEST['FilterOperator'][1]=='<=>' ? (get_magic_quotes_gpc() ? stripslashes($_REQUEST['FilterValue'][1]) : $_REQUEST['FilterValue'][1]) : "");
	...
	<script>
		...
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint){ ?>
			...
			$j("#academicyear-container__RAND__").select2({
				...
				initSelection: function(e, c){
					$j.ajax({
						...
						success: function(resp){
							...
							if(resp.results[0].id == '<?php echo empty_lookup_value; ?>'){ $j('.btn[id=academic_year_view_parent]').hide(); }else{ $j('.btn[id=academic_year_view_parent]').show(); }
				...
				if(e.added.id == '<?php echo empty_lookup_value; ?>'){ $j('.btn[id=academic_year_view_parent]').hide(); }else{ $j('.btn[id=academic_year_view_parent]').show(); }
	...
	$lookup_fields = array(  'course' => array('courses', 'Course'), 'academicyear' => array('academic_year', 'Academicyear'));
