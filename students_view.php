<?php
	...
	$x->filterers = array(  'course' => 'Course', 'academicyear' => 'Academicyear');

	$x->QueryFrom = "`students` LEFT JOIN `courses` as courses1 ON `courses1`.`id`=`students`.`course` LEFT JOIN `academic_year` as academic_year1 ON `academic_year1`.`id`=`students`.`academicyear` ";
	$x->QueryWhere = '';
	$x->QueryOrder = '';
	...
	$x->ColCaption = array("Regno", "Name", "Course", "Year of study", "Academicyear");
	$x->ColFieldName = array('regno', 'name', 'course', 'year', 'academicyear');
